<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table
      ...
      <el-table-column min-width="120" align="center" label="预约开始时间">
        <template slot-scope="scope">
          <span>{{ scope.row.startTime }}</span>
        </template>
      </el-table-column>
      <el-table-column min-width="120" align="center" label="预约结束时间">
        <template slot-scope="scope">
          <span>{{ scope.row.endTime }}</span>
        </template>
      </el-table-column>
       <el-table-column min-width="150" align="center" label="预约检测站">
        <template slot-scope="scope">
          <span>{{ scope.row.detectionBranchName }}</span>
        </template>
      </el-table-column>
...
<script>
import { fetchYearAuditApplyList } from '@/api/car/car'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchYearAuditApplyList(this.listQuery).then(response => {
