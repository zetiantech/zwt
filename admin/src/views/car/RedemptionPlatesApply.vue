<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table
      ...
      <el-table-column min-width="120" align="center" label="是否需要临时牌">
        <template slot-scope="scope">
          <span>{{ scope.row.needTmpPlateNo | filtersTmpPlateNo }}</span>
        </template>
      </el-table-column>
...
<script>
import { fetchMgmtRptPlateApplyList } from '@/api/car/car'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchMgmtRptPlateApplyList(this.listQuery).then(response => {
