<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table
      ...
      <el-table-column min-width="120" align="center" label="身份证号">
        <template slot-scope="scope">
      ...
      <el-table-column min-width="120" align="center" label="业务类型">
        <template slot-scope="scope">
          <span>{{ scope.row.busTypeId }}</span>
        </template>
      </el-table-column>
      <el-table-column min-width="120" align="center" label="号牌种类">
        <template slot-scope="scope">
          <span>{{ scope.row.plateKindId }}</span>
        </template>
      </el-table-column>
      <el-table-column min-width="120" align="center" label="车牌号码">
        <template slot-scope="scope">
          <span>{{ scope.row.plateNo }}</span>
        </template>
      </el-table-column>
      <el-table-column min-width="120" align="center" label="车架号">
        <template slot-scope="scope">
          <span>{{ scope.row.vin }}</span>
        </template>
      </el-table-column>
       <el-table-column min-width="120" align="center" label="机动车引擎号">
        <template slot-scope="scope">
...
<script>
import { fetchHandleIllegalApplyList } from '@/api/car/car'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchHandleIllegalApplyList(this.listQuery).then(response => {
