    ...
    async authenticate() {
        ...
        try {
            ...
            if (!hardware || !permission || !enrolled) {
                let message = !enrolled ? '没有注册指纹' : !hardware ? '该设备不支持指纹扫描' : '没有权限'
                this.setState({
                    phase: 'fail',
                    message
                });
        ...
        this.props.fingerprintResult(this.state)
    }
            ...
            <View style={{flex:1}}>
                <View duration={1000} transition="backgroundColor"
                    ...
                    <Text style={styles.welcome}>
                        {this.state.phase === 'normal' && '“政务通”的触控ID'}
                        {this.state.phase === 'warn' && '再试一次！'}
                        {this.state.phase === 'fail' && '验证失败！'}
                        {this.state.phase === 'success' && '验证成功！'}
                    </Text>
