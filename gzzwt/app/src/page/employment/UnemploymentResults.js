...
export default function UnemploymentResults (props) {
    ...
    useEffect(() => {
        ...
        if (info !== null) {
            info.status = props.navigation.getParam('info', {}).status === 2 ? '审核通过' : '审核中';
            setInfoData({ ...infoData, ...info })
        }
    }, [])
    return (
        <View style={styles.container}>
            <NavigationBar title='个人失业登记信息查询结果' hide={false} navigator={props.navigation} popEnabled={true} />
            <WhiteSpace size="lg" />
