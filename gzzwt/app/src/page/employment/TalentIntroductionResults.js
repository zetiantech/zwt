...
export default function TalentIntroductionResults (props) {
    ...
    useEffect(() => {
        ...
        if (info !== null) {
            if (props.navigation.getParam('info', {}).status) info.status = props.navigation.getParam('info', {}).status === 2 ? '审核通过' : '审核中';
            setInfoData({ ...infoData, ...info })
        }
    }, [])
    return (
        <View style={styles.container}>
            <NavigationBar title='人才引进接收情况查询结果' hide={false} navigator={props.navigation} popEnabled={true} />
            <WhiteSpace size="lg" />
            <ScrollView >
