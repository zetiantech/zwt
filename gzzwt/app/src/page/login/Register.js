...
import { 
 ...
 const RegisterComponent = ({params, setParams, checked, codeText, onSendCode, onSubmitLogin, props}) => {
	...
	return (
		<View style={styles.LoginBox}>
			...
			<View style={{marginTop: 30, marginLeft: 15}}>
				<Button type="primary" onPress={onSubmitLogin}>注册</Button>
			</View>
			<View style={styles.agreenText}>
				<Text style={{color: '#999999'}}>注册即同意</Text>
				<TouchableOpacity>
 ...
 export default function Register(props){
	...
	function onSubmitLogin(){
	  ...
	  if (validator.isEmpty(shortCode)) {
		...
		HttpUtil.post(API.URL_REGISTER, {
			phone: phone,
			shortCode: shortCode
		}).then((responseJson) => {
			const { code, data, msg } = responseJson.data
			...
			if(code == 0 ) {
			  ...
			  if(isTouchID){
				NavigationUtil.navigate(props, 'UnlockMode')
			  }else{
	 ...
	 return (
		<Provider>
			<View style={styles.container}>
				...
				/>
				<LoginFooter type={1} style={{marginTop: 80}} props={props}/>
			</View>
		</Provider>
	 )
 }
