    ...
    return (
        <View style={styles.infoBox}>
            <View style={styles.infoTop}>
                ...
                <Text style={styles.address}>{data.address ||'广州市'}</Text>
            </View>
...
export default function DrugstoreAddress(props) {
    const [coords, setCoords] = useState({
        latitude: 23.10623,
        longitude: 113.323656,
    }); // 定位坐标
    const [MapFlag, setMapFlag] = useState(false);
    const data = props.navigation.getParam('info', {});//参数,经纬度
        ...
        <Provider>
            <View style={styles.container}>
                ...
                <View style={styles.mapWrap}>
                    {coords && <MapContainer coords={coords} data={data} />}
