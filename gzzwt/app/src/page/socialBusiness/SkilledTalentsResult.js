 ...
 * @param {*} param0 
 */
function RenderContent({ data, labelData }) {
    return (
        <View style={styles.formBox}>
            <Form style={{ borderRadius: 10, overflow: 'hidden' }} data={data} showData={labelData} />
        </View>
    )
}
/**
 ...
 */
const ScrollViewer = ({ getData, setListView, labelData }) => {
    return (
        <ListView
            // refreshable={false}
            ...
            renderItem={(item, index) => {
                return (
                    <RenderContent data={item} labelData={labelData} />
                )
            }}
            numColumns={1}
        />
...
export default function SkilledTalentsResult(props) {
...
*/
    function getData(page = 1, startFetch, abortFetch) {
        ...
        HttpUtil.post(API.GETTECHNIQUETALENTS, {
            ...infoData,
            page: page,
            size: 10
        }).then((data) => {
            ...
            <View style={styles.container}>
                ...
                {infoData ? (
                    <ScrollView styles={styles.ListWrap}>
                        <ScrollViewer getData={getData} labelData={labelData} setListView={setListView} />
                    </ScrollView>
