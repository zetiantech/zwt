    ...
    function handleSubmit() {
        validate({ data, showData, setShowData })
            .then(() => {
                let params = {
                    ...data,
                    bankTypeId: data.bankTypeId.join(',')
                };
                console.log(API.GETBANKRESERVE, '---')
                HttpUtil.post(API.GETBANKRESERVE, params).then((data) => {
                    data = data.data;
                    if (data.code === 0 && data.data !== null) {
                        let infoData = data.data;
                        NavigationUtil.navigate(props, 'QueryFinanceResult', {
                            info: infoData
                        })
