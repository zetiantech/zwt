        ...
        <View style={styles.filterBox}>
            ...
            <View style={[styles.filterItem]}>
                <Picker
                    ...
                    data={levelData}
                    value={levelId}
                    onChange={(val) => { setLevelId(val[0]); changeHospitals(val, 2) }}
                    onOk={(val) => { setLevelId(val[0]); changeHospitals(val, 2) }}
                >
                    {/* <Text>级别</Text> */}
                    <TouchableOpacity>
                        <Text style={styles.filterText}>
...
function ListWrap({ data, props }) {
    return (
        <TouchableOpacity
            onPress={() => { NavigationUtil.navigate(props, 'SubscribeRegist1', { data: data }) }}
        >
                ...
                <View style={styles.listItem}>
                    <View style={styles.itemLeft}>
                        <View style={styles.hospitalsName}>
                            <Text>{data.name}</Text>
                            <Text style={styles.levels}>{data.levelName.substring(0, 4)}</Text>
                        </View>
                    ...
                    <View>
                        <Text style={{ color: '#2F74ED' }}>{data.distance || 0} km</Text>
                        {data.distance <= 2 &&
                            <Text style={{ color: '#999' }}>距离最近</Text>
                        }
...
export default function SubscribeRegist(props) {
    const [keyword, setKeyword] = useState();
    ...
    */
    function getData(page = 1, startFetch, abortFetch) {
        ...
        console.log(coords.latitude, 999)
        console.log(coords.longitude, 999)
        HttpUtil.get(API.HospitalList, {
            ...params,
            ...
            data = data.data;
            if (data.code === 0) {
                console.log(data.data.list, 44444444)
    ...
    console.log(params, 333333333)
    return (
        <Provider>
            <View style={styles.container}>
                <NavigationBar title='预约挂号' hide={false} navigator={props.navigation} popEnabled={true}
