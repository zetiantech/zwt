...
export default function MedicalPractitioners(props) {
    ...
    function onSubmitLogin() {
        validate({ data, showData, setShowData })
            .then(() => {
                HttpUtil.post(API.GetDoctorCertificates, {
                    ...data,
                }).then((data) => {
                    data = data.data;
                    if (data.code === 0) {
                        NavigationUtil.navigate(props, 'MedicalResults', { info: data.data || null });
                    } else {
                        setToastFlag(true);
    ...
    return (
        <Provider>
            <View style={styles.container}>
                <NavigationBar title='职业医师资格证查询' hide={false} navigator={props.navigation}
                    popEnabled={true} />
                <WhiteSpace size="lg" />
