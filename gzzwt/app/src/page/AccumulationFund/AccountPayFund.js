 ...
 * startYears-起始年月  endYears-结束年月, depositeMonths-缴存月数, thisMoney-本次应收金额,depositeMoney-汇缴托收金额
*/
export default function TransferTheBooking  (props) {
    ...
    const [depositeMonths,setdepositeMonths] = useState() //缴费月份
    const [monthsSum,setmonthsSum] = useState() //月缴余额
    const [collectMoney,setcollectMoney] = useState() //本次应收金额


    const [papersType,setpapersType] = useState() //身份证ID

    let types = props.navigation.getParam('type')||"1"
    //计算月数
    function datemonth(){
        // 拆分年月日
        var date1 = JSON.stringify(formData.startYears).substring(1,11).split('-')||"";
        // 得到月数
        var date11 = parseInt(date1[0]) * 12 + parseInt(date1[1])||"";
        // 拆分年月日
        var  date2 = JSON.stringify(formData.endYears).substring(1,11).split('-')||""   
        // 得到月数
        var date22 = parseInt(date2[0]) * 12 + parseInt(date2[1]);
        var m = Math.abs(date22 - date11)||"0";
        setdepositeMonths(m)
    }
    /**
     * 计算缴存金额
    ...
    function overallMoney(){
        ...
        if (formData.depositeCarnumber!=""&&formData.perageRatio!="") {
        let overall = JSON.stringify(Math.abs(formData.depositeCarnumber*formData.perageRatio*depositeMonths))||""
        setcollectMoney(overall)      
        }    
    }
    useEffect(() => {
        datemonth()
        depositeMoney()
        overallMoney()
    }, [formData,depositeMonths,monthsSum])
    ...
    function onClickbtn () {
        ...
        if (formData.startYears==''||formData.startYears==undefined) {
            ToastUtil.toast('请选择起始年月')   
            return  
        }
        if (formData.endYears==''||formData.endYears==undefined) {
            ToastUtil.toast('请选择结束年月')   
            return  
        }
        //获取汇缴用户信息
        HttpUtil.post('http://192.168.10.58:8830/fund/accountRemit/add',          
            {accessToken:'eyJhbGciOiJIUzI1NiJ9.eyJqdGkiOiI2Iiwic3ViIjoie1wiY3JlYXRlVGltZVwiOjE1NzI4NTI5MDIwMDAsXCJnZW5kZXJcIjowLFwiaWRcIjo2LFwiaXNBY3RpdmVcIjoxLFwiaXNDZXJ0aWZ5XCI6MCxcImlzRmluZ2VycHJpbnRcIjowLFwibmFtZVwiOlwi5pa55a-.e2blxMrBr_dh7NrFvppBlNIPbDtSNyAa7-2poy3g530',
                name:formData.name,
                idType:1,
                ...
                startRemitTime:dayjs(formData.startYears).format('YYYY-MM-DD hh:mm:ss'),
                endRemitTime:dayjs(formData.endYears).format('YYYY-MM-DD hh:mm:ss'),
                remitMonths:depositeMonths||"",
                receivableAmount:collectMoney||"",
                remitCollectionAmount:collectMoney||"",              
            }     
            ).then(responseJson=>{       
    ...
    // console.log(formData,6666666666)  
        return (
            <View style={styles.container}>
                ...
                <View>                    
                        <Text style={styles.listTitle} >本次汇缴信息</Text>
                    <List>       
                        <DatePicker
                            ...
                            maxDate={new Date}
                            onChange={(val)=>setFormData({...formData, startYears: val})}
                            format="YYYY-MM-DD"
                            >
                            <List.Item arrow="horizontal">起始汇缴年月</List.Item>
                        </DatePicker>                      
                        <DatePicker
                            ...
                            minDate={new Date(2000, 7, 6)}
                            maxDate={new Date}
                            onChange={(val)=>setFormData({...formData, endYears: val})}
                            format="YYYY-MM-DD"
                            >
                            <List.Item arrow="horizontal">结束汇缴年月</List.Item>
                        </DatePicker>
                        <InputItem
                            labelNumber={8}
                            type="number"
                            value={depositeMonths+""}
                            // onChange={(val)=>setFormData({...formData, depositeMonths: val})}
                            textAlign='right'
                        >
                            本次汇缴月数
                        </InputItem> 
