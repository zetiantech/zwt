...
export default function dateReplacement2(props) {
    ...
    const [SelectDataThree, setSelectDataThree] = useState([ //邮寄类型
        {value: 1,label: '委托邮政快递'}, 
        {value: 2,label: '前往窗口自取'},
    ]);
    ...
    useEffect(() => {
        //业务类型
        HttpUtil.get(API.KeyValueList,{kindId:16})  
            .then(responseJson=>{   
    ...
    //下一步
    function onBtnClick () {
        let parmsID = {applyingID:applyingID,bustyID:bustyID,threeID:threeID,}
        let parmsText = {busText:busText,applyingText:applyingText}
        let addRessIdData = {checked:checked,ressData:ressData}//网点ID和快递地址
        ...
        if (threeID==""||threeID==undefined) {
            ...
            if (ressData==undefined&&ressData=='') { //判断是否邮寄
                NavigationUtil.navigate(props, 'EntrustConfirmation',
                {formImgData:formImgData,parmsID:parmsID,parmsText:parmsText}
                );
            }else{
                NavigationUtil.navigate(props, 'EntrustConfirmation',
                {formImgData:formImgData,addRessIdData:addRessIdData,parmsID:parmsID,parmsText:parmsText}
                );
            }
        }
