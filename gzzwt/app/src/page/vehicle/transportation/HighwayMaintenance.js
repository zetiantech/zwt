...
export default function HighwayMaintenance(props) {
        ...
        validate({ data, showData, setShowData })
            .then(() => {
                HttpUtil.post(API.CuringQuery, {
                    ...
                    if (data.code === 0) {
                        setToastFlag(false);
                        NavigationUtil.navigate(props, 'HighwayMaintenanceResults', {
                            info: data.data || null
                        });
                    } else {
                        setToastFlag(true);
    ...
    return (
        <Provider>
            <View style={styles.container}>
                <NavigationBar title='公路养护查询' hide={false} navigator={props.navigation}
                    popEnabled={true} />
                <WhiteSpace size="lg" />
