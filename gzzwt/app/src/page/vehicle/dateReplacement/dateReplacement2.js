...
import RadioView from 'src/component/RadioViewImage'
import AMAPComponent from 'src/component/AMAPComponent'
...
export default function dateReplacement2(props) {
    ...
    useEffect(() => {
        //业务类型
        HttpUtil.get(API.KeyValueList,{kindId:15})  
            .then(responseJson=>{   
    ...
    function onBtnClick () {
        ...
        if (threeID==""||threeID==undefined) {
            ...
            if (ressData==undefined&&ressData=='') { //判断是否邮寄
                NavigationUtil.navigate(props, 'dateinformation',
                {params:params,applyingText:applyingText,busText:busText,applyingID:applyingID,bustyID:bustyID,threeID:threeID}
                );
            }else{
                NavigationUtil.navigate(props, 'dateinformation',
                {params:params,checked:checked,applyingText:applyingText,busText:busText,ressData:ressData,applyingID:applyingID,bustyID:bustyID,threeID:threeID}
                );
            }
        }
